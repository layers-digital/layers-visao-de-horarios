export default function formatTime(time) {
  ...
  if (Object.prototype.toString.call(time) === "[object Date]") {
    ...
    if (
      timeTransformedInDate instanceof Date &&
      !isNaN(timeTransformedInDate)
    ) {
      const hours =
        (timeTransformedInDate.getHours() < 10 ? "0" : "") +
        timeTransformedInDate.getHours();
      const minutes =
        (timeTransformedInDate.getMinutes() < 10 ? "0" : "") +
        timeTransformedInDate.getMinutes();
      return hours + ":" + minutes;
    }
